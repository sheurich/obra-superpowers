...
for path in "$README_PI" "$INSTALL_DOC" "$AGENTS_DOC" "$ROOT_README" "$REVIEWER"; do
...
grep -q '`code-reviewer`' "$README_PI" || fail "docs/README.pi.md does not document the required code-reviewer agent"
...
grep -q 'code-reviewer' "$INSTALL_DOC" || fail ".pi/INSTALL.md does not include code-reviewer installation"
...
grep -q 'subagent support by itself' "$AGENTS_DOC" || fail ".pi/agents/README.md does not explain that code-reviewer is not subagent support by itself"
grep -q '`worker` agent' "$AGENTS_DOC" || fail ".pi/agents/README.md does not mention the external worker agent dependency"
pass ".pi/agents/README.md scopes bundled agents honestly"
...
grep -q '^name:' "$REVIEWER" || fail "code-reviewer agent is missing name frontmatter"
grep -q '^description:' "$REVIEWER" || fail "code-reviewer agent is missing description frontmatter"
pass "code-reviewer agent profile remains valid"
